...
This library provides a middle ground. It stores *O*(log n) states while still allowing pushing actions in *O*(1) (amortized) time and popping actions in *O*(log n) time, without requiring invertible actions. Furthermore, it also allows reconstructing the state at any point in the history in *O*(k + log n) time[^2] where k is the age of the state.
...
impl history::Action for MyAction {
	...
	fn apply(&self, state: i32) -> Result<i32, Self::Error> {
		Ok(match self {
